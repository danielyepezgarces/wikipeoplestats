...
export class Database {
  ...
  static async blacklistToken(tokenHash: string, userId: number, reason: string): Promise<void> {
    const conn = await this.getConnection()
    try {
      // Create token_blacklist table if it doesn't exist
      await conn.execute(`
      ...
      // Set expiration to 30 days from now (same as JWT expiration)
      const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)

      await conn.execute("INSERT INTO token_blacklist (token_hash, user_id, reason, expires_at) VALUES (?, ?, ?, ?)", [
        tokenHash,
        userId,
        reason,
    ...
    try {
      const [rows] = await conn.execute(
        `SELECT * FROM sessions 
         WHERE user_id = ? AND id != ? AND is_active = TRUE AND expires_at > NOW()
