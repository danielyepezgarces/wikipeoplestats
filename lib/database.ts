...
export class Database {
  ...
  static async getUserByWikipediaId(wikimediaId: string): Promise<User | null> {
    ...
    try {
      const [rows] = await connection.execute(
        "SELECT * FROM users WHERE wikimedia_id = ?",
        [wikimediaId]
      );
  ...
  static async claimUserAccount(
    ...
    try {
      await connection.execute(
        "UPDATE users SET wikimedia_id = ?, email = ?, is_claimed = TRUE WHERE id = ?",
  ...
  static async revokeRefreshToken(tokenJti: string): Promise<void> {
    ...
    try {
      await connection.execute(
        "UPDATE refresh_tokens SET is_revoked = TRUE WHERE token_jti = ?",
        [tokenJti]
      );
    } finally {
      connection.release();
